  ...
  (nextConfig = {}) => {
    ...
    return {
      ...
      webpack(config, options) {
        const { webpack, buildId, dev } = options;

        console.log(
          `> [PWA] Compile ${options.isServer ? "server" : "client (static)"}`
        );

        let { runtimeCaching = defaultCache, ...restPluginOptions } =
          pluginOptions;
        const {
          scope = basePath,
          sw = "sw.js",
        ...
        config.plugins.push(
          new webpack.DefinePlugin({
            __PWA_SW__: `'${path.posix.join(basePath, sw)}'`,
            __PWA_SCOPE__: `'${_scope}'`,
            __PWA_ENABLE_REGISTER__: `${Boolean(pluginOptions.register)}`,
            __PWA_START_URL__: pluginOptions.dynamicStartUrl
              ? `'${basePath}'`
              : undefined,
            __PWA_CACHE_ON_FRONT_END_NAV__: `${Boolean(
              pluginOptions.cacheOnFrontEndNav
            )}`,
            __PWA_RELOAD_ON_ONLINE__: `${Boolean(pluginOptions.reloadOnOnline)}`
          })
        );
        ...
        if (!options.isServer) {
          const _dest = path.join(options.dir, distDir);
          const customWorkerScriptName = buildCustomWorker({
            id: buildId,
            basedir: options.dir,
            customWorkerDir: pluginOptions.customWorkerDir || "worker",
            destdir: _dest,
          ...
          if (!!customWorkerScriptName) {
            pluginOptions.importScripts.unshift(customWorkerScriptName);
          }

          if (pluginOptions.register) {
            console.log(
          ...
          console.log(`> [PWA]   url: ${path.posix.join(basePath, sw)}`);
          ...
          let manifestEntries = pluginOptions.additionalManifestEntries;
            ...
            manifestEntries = globby
              .sync(
                [
                  ...
                  `!${sw.replace(/^\/+/, "")}`,
                  `!${sw.replace(/^\/+/, "")}.map`,
                  ...(pluginOptions.publicExcludes || ["!noprecache/**/*"])
                ],
                {
                  cwd: "public"
                }
          ...
          if (pluginOptions.cacheStartUrl) {
            if (!pluginOptions.dynamicStartUrl) {
              manifestEntries.push({
                url: basePath,
                revision: buildId
              });
            } else if (
              typeof pluginOptions.dynamicStartUrlRedirect === "string" &&
              pluginOptions.dynamicStartUrlRedirect.length > 0
            ) {
              manifestEntries.push({
                url: pluginOptions.dynamicStartUrlRedirect,
                revision: buildId
              });
            }
          }

          let _fallbacks = pluginOptions.fallbacks;
          ...
          if (_fallbacks) {
            ...
            if (res) {
              ...
              pluginOptions.importScripts.unshift(res.name);
          ...
          if (workbox.swSrc) {
            ...
            if (dev) {
              ...
              pluginOptions.ignoreURLParametersMatching.push(/ts/);
              runtimeCaching = [
                {
                  urlPattern: /.*/i,
                  handler: "NetworkOnly",
            ...
            config.plugins.push(
              new WorkboxPlugin.GenerateSW({
                ...workboxCommon,
                skipWaiting: pluginOptions.skipWaiting,
                clientsClaim: pluginOptions.clientsClaim,
                cleanupOutdatedCaches: pluginOptions.cleanupOutdatedCaches,
                ignoreURLParametersMatching:
                  pluginOptions.ignoreURLParametersMatching,
